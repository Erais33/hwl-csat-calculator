...
st.markdown(
    "Upload a CSV with **Date**, **Ratings**, sub-category scores, and **Comments**.\n\n"
    "Expected columns: Date, Ratings, Value For Money, Security, Location, Staff, "
    "Atmosphere, Cleanliness, Facilities, Comment."
)
...
if uploaded_file:
    ...
    # ---- SUB-CATEGORIES ----
    subs = ['Value For Money', 'Security', 'Location', 'Staff',
    ...
    lowest_value = sub_scores[lowest_sub]
    st.info(f"💡 Focus tip: *{lowest_sub}* is your lowest sub-score at {lowest_value:.2f}. This is your best area to improve.")
    ...
    # ---- COMMENTS INSIGHT ----
    st.subheader("📝 Comments Insights")
    ...
    expected_new_avg = st.number_input(
        "✅ Realistic average for new reviews:", value=9.2, min_value=0.1, max_value=10.0, step=0.1, format="%.2f"
    )
    ...
    if expected_new_avg > target_avg:
        ...
        st.info(
            ...
            f"averaging {expected_new_avg:.2f} / 10.00 (total points needed: {total_score_needed:.0f})."
        )
    else:
    ...
    st.info("📂 Upload your CSV to begin your full CSAT insights forecast.")
