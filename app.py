...
st.markdown(
    ...
    <style>
    .stApp {
        background-color: #000000; /* Black outer background */
    }

    .main-container {
        background-color: #FFA726; /* Darkish orange */
...
with st.container():
    ...
    if uploaded_file:
        try:
            ...
            st.write("Pick the date you want to forecast. Reviews older than 6 months drop off on that date.")
            ...
            # ---- FORECAST RESULTS ----
            st.header("📈 Forecast")
            st.write(f"📉 If you add no new reviews, your rolling average would drop to: {new_avg_if_none:.2f} / 10.00"_
