...
st.markdown("Upload a CSV file that includes **Date** and **Ratings** columns. Example: each row should have when the review was posted and its score (0–10).")
...
if uploaded_file:
    try:
        ...
        st.header("🧾 Summary")
        st.success(f"✅ **Reviews staying:** {current_reviews_count} | Average: {current_avg:.2f} / 10")
        st.info(f"🔻 **Reviews dropping off:** {dropped_reviews_count} | Average: {dropped_reviews_avg:.2f} / 10")
        ...
        st.header("📊 Forecast")
        st.success(
            f"📉 **If you add no new reviews**, your rolling average would drop to: **{new_avg_if_none:.2f} / 10**"
        )
        if new_reviews > 0:
            st.info(
                f"⭐️ **To reach {target_avg:.2f}**, your new reviews must average: **{required_new_avg:.2f} / 10**"
            )
        else:
